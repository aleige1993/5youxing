...
(function ($, document) {
  ...
  const PopPicker = $.PopPicker = $.Class.extend({
    ...
    init(options) {
      ...
      self.cancel.addEventListener('tap', (event) => {
        self.hide();
      }, false);
      self.ok.addEventListener('tap', (event) => {
        if (self.callback) {
      ...
      self.mask[0].addEventListener('tap', () => {
        self.hide();
      }, false);
      self._createPicker();
      // 防止滚动穿透
      self.panel.addEventListener($.EVENT_START, (event) => {
        event.preventDefault();
      }, false);
      self.panel.addEventListener($.EVENT_MOVE, (event) => {
        event.preventDefault();
      }, false);
    },
    _createPicker() {
...
(function ($, document) {
  ...
  const DtPicker = $.DtPicker = $.Class.extend({
    init(options) {
      ...
      ui.cancel.addEventListener('tap', () => {
        self.hide();
      }, false);
      ui.ok.addEventListener('tap', () => {
        const rs = self.callback(self.getSelected());
        if (rs !== false) {
          self.hide();
        }
      }, false);
      ui.y.addEventListener('change', (e) => { // 目前的change事件容易导致级联触发
      ...
      ui.m.addEventListener('change', (e) => {
        self._createDay();
      }, false);
      ui.d.addEventListener('change', (e) => {
        if (self.options.beginMonth || self.options.endMonth) { // 仅提供了beginDate时，触发day,hours,minutes的change
          self._createHours();
        }
      }, false);
      ui.h.addEventListener('change', (e) => {
        if (self.options.beginMonth || self.options.endMonth) {
          self._createMinutes();
        }
      }, false);
      ui.mask[0].addEventListener('tap', () => {
        self.hide();
      }, false);
      self._create(options);
      // 防止滚动穿透
      self.ui.picker.addEventListener($.EVENT_START, (event) => {
        event.preventDefault();
      }, false);
      self.ui.picker.addEventListener($.EVENT_MOVE, (event) => {
        event.preventDefault();
      }, false);
    },
    getSelected() {
