  ...
  <div id="couponList" class="mui-content mui-scroll-wrapper">
    <div class="mui-scroll">
      ...
      <div class="coupon-list">
        <p v-if="!couponList.length" class="no-data">暂无优惠券</p>
        <ul v-if="couponList.length">
          <li v-for="(list, index) in couponList" :key="index">
            <div  :class="'main ' + list.className">
              <!-- <span @tap="couponShare(list.couponName, list.activityNo)" class="share"></span> -->
              <img v-if="list.status === '1'" class="status" src="../assets/coupon-used.png" alt="">
              <img v-if="list.status === '2'" class="status" src="../assets/coupon-exchanged.png" alt="">
              <img v-if="list.status === '3'" class="status" src="../assets/coupon-timeout.png" alt="">
              <!--couponType-->
              ...
              <div class="text">
                ...
                <p @tap="showExplain(index)">
                  <span>
                    使用说明
                    <img v-if="list.status === '0'" src="../assets/bottom-arrow-solid2.png" alt="">
                    <img v-else src="../assets/bottom-arrow-solid.png" alt="">
                  </span>
                </p>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    //   const username = this.$Tools.stringAddStar(this.$store.state.user.mobile);
    //   const title = `${username}用户获得优惠劵"${couponName}"`;
    //   const desc = `${username}用户邀您到店参与颂车小站开业抽奖活动`;
    //   const url = `${window.location.origin}/#/draw/begin?id=${activityNo}&memberNo=${this.$store.state.user.mobile}`;
    ...
    async initData() {
      const res = await this.$postData(`activity/travel/list/myCoupon?memberNo=${this.$store.state.user.memberNo}`);
      this.$data.couponList = res;
...
<style lang="less" scoped>
  ...
  .coupon-list .main.white {
    background: #fff url("../assets/wave-white.png") left bottom no-repeat;
    background-size: 100%;
  }
  .coupon-list .main.orange {
    background: #ffc939 url("../assets/wave-orange.png") left bottom no-repeat;
    background-size: 100%;
  }
  .coupon-list .main.blue {
    background: #3984ff url("../assets/wave-blue.png") left bottom no-repeat;
    background-size: 100%;
  }
  .coupon-list .main.green {
    background: #25d171 url("../assets/wave-green.png") left bottom no-repeat;
    background-size: 100%;
  }
  .coupon-list .main.gray {
    color: #fff;
    background: #bfbfbf url("../assets/wave-gray.png") left bottom no-repeat;
    background-size: 100%;
  }
